...
def recommended_from_search(request):
	#get the common words from stored searches
...
def frequent_search_words(request):
	...
	# join all of the searches toguether into a single string
...
# views
def log_product_views(request, product):
...
def recommended_from_views(request):
	t_id = tracking_id(request)
	#get recently viewed products by this customer	
	...
	if viewed:
		...
		#if there are other tracking ids, get other products, not just the ones associated with the current customer
		if t_ids:
